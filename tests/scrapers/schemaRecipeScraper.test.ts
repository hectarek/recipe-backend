...
import { extractRecipeFromHtml } from '../../src/scrapers/schemaRecipeScraper.js';
...
describe('extractRecipeFromHtml', () => {
  it('parses recipe information from JSON-LD', () => {
    const result = extractRecipeFromHtml(SAMPLE_HTML, 'https://example.com/recipe');
  ...
  it('throws when no recipe schema is present', () => {
    expect(() => extractRecipeFromHtml('<html></html>', 'https://example.com')).toThrow(
      /No recipe schema/i
    );
  });
});
