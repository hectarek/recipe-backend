...
import { createScrapeRecipeHandler } from '../../src/routes/scrapeRecipe.js';
import type { RecipeIntakeOptions, RecipeIntakeResponse } from '../../src/types.js';
...
type HandleStub = {
  fn: (url: string, options?: RecipeIntakeOptions) => Promise<RecipeIntakeResponse>;
  calls: HandleCall[];
...
const createHandleStub = (): HandleStub => {
  ...
  return {
    ...
    fn: async (url: string, options: RecipeIntakeOptions = {}) => {
...
const invokeRoute = async (handler: (request: Request) => Promise<Response>, body: unknown) => {
...
describe('POST /scrape-recipe', () => {
  ...
  it('returns 200 with structured payload', async () => {
    handleStub.setResponse({
      recipe: { title: 'Test', sourceUrl: 'https://example.com', instructions: 'Step 1' },
      ingredients: [],
      unmatched: [],
      rawSchema: {}
    });
