...
const getPlainText = (value: { plain_text: string }[] | undefined): string | undefined =>
  value
...
export class NotionClient implements NotionGateway {
  ...
  private get databaseQuery() {
    return (
      this.client.databases as unknown as {
        query: (args: { database_id: string; start_cursor?: string }) => Promise<{
  ...
  constructor(options: NotionGatewayOptions) {
    ...
    if (!apiToken) {
      throw new Error('NOTION_API_TOKEN is required to initialize NotionClient.');
    }
  ...
  private mapFoodPage(page: PageObjectResponse): FoodLookupItem | null {
    ...
    const namePropertyKey = mappings.foodName ?? defaultPropertyMappings.foodName;
    const fallbackNameProperty = 'Name' in properties ? properties.Name : undefined;
    ...
    const aliasesPropertyKey = mappings.foodAliases ?? defaultPropertyMappings.foodAliases;
  ...
  async createRecipePage(recipe: ScrapedRecipe): Promise<string> {
    ...
    const recipeNameProperty = mappings.recipeName ?? defaultPropertyMappings.recipeName;
  ...
  async createIngredientEntries(recipePageId: string, ingredients: MatchedIngredient[]) {
    if (!this.options.ingredientDatabaseId) {
      throw new Error('ingredientDatabaseId is not configured.');
    }

    const mappings = this.options.propertyMappings ?? defaultPropertyMappings;
    const recipeRelationProperty =
      mappings.ingredientRecipeRelation ?? defaultPropertyMappings.ingredientRecipeRelation;
    const foodRelationProperty =
      mappings.ingredientFoodRelation ?? defaultPropertyMappings.ingredientFoodRelation;
    const quantityProperty =
      mappings.ingredientQuantity ?? defaultPropertyMappings.ingredientQuantity;
    const unitProperty = mappings.ingredientUnit ?? defaultPropertyMappings.ingredientUnit;
    const ingredientNameProperty =
      mappings.ingredientName ?? defaultPropertyMappings.ingredientName;

    for (const ingredient of ingredients) {
      const properties: Record<string, unknown> = {
        [recipeRelationProperty]: {
      ...
      if (ingredient.foodId) {
        properties[foodRelationProperty] = {
          relation: [{ id: ingredient.foodId }]
        };
      }

      if (quantityProperty) {
        properties[quantityProperty] = {
