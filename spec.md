...
Success criteria
    - Scrapes most mainstream recipe sites without custom per-site logic.
    - Produces ingredient objects with at least raw, name, and qty for 80% of lines.
    - Returns a list of unmatched ingredients for manual cleanup.
    - JSON shape is stable so the Notion-creation step can be a separate script.
...
/src
  ...
  /parsers
    ingredientParser.ts        // "1 cup chopped onion" -> {qty, unit, name}
  /matchers
    foodMatcher.ts             // name -> Food Lookup item
  /services
    notionClient.ts            // (optional) to fetch Food Lookup / push rows
  /routes
    scrapeRecipe.ts
  index.ts
```
...
function matchIngredientToFood(
